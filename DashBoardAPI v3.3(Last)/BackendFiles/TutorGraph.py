...
class TutorGraph():
    ...
    def PreprocessData(self, stdAttandance, stdReg, stdData, staffData, staffAttandance, staffWorkGroup, stdSchoolData):
        ...
        stdData = stdData.merge(stdReg, on='StudentID', how='right')
    ...
    def GetTotalStudentByInstitute(self, stdAttandance, instituteList, filterType, dateFrom, dateTo):
        ...
        filteredDF23 = self.stdDateAttandance
        filteredDF23.AttDate = pd.to_datetime(filteredDF23.AttDate)
        dateFrom2 = pd.to_datetime(GetDateFrom(dateFrom))
        dateTo2 = pd.to_datetime(GetDateTo(dateTo))
        filteredDF23 = filteredDF23[filteredDF23.AttDate >= dateFrom2]
        filteredDF23 = filteredDF23[filteredDF23.AttDate <= dateTo2]
        ...
        for institute, df in instituteFilteredDF.items():
            ...
            count = finalDF.PresentInFull.sum()
            temp23 = filteredDF23[filteredDF23.Branch.str.contains(institute)]
            temp28 = len(temp23[temp23.AttCode == 2])
            count = count + temp28
            if math.isnan(count):
                count = 0
            if len(withDateDF) > 0:
        ...
        for key in studentsByInstitute.keys():
            ...
            if totalNum > 0:
                ...
                if math.isnan(val):
                    ...
                    pupilPerTutorByInstitute[key] = math.ceil(val)
            else:
                pupilPerTutorByInstitute[key] = 0
...
# tGraph = TutorGraph()
# print(tGraph.GetNumberOfPupilPerTutorByInstitute(dic.values(), 'Branch', '2017-10-09', '2017-12-08', 'a'))
